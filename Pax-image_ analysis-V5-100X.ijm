...
run("Unsharp Mask...", "radius=11 mask=0.8 stack");
run("Convolve...", "text1=[1 2 3 2 1\n2 6 9 6 2\n3 9 81 9 3\n2 6 9 6 2\n1 2 3 2 1] normalize stack");
...
run("Analyze Particles...", "size=750-Infinity show=Masks slice");
...
selectWindow("Summary of Pax");//Table.rename("Summary", SaveName+"_Tot-Pax");
Table.save(outputData+SaveName+"_Tot-Sum-Pax.csv");run("Clear Results");Table.deleteRows(0, 15, "Summary of Pax");
...
run("Options...", "iterations=100 count=1 black do=Erode");run("Options...", "iterations=25 count=1 black do=Erode");
run("Create Selection"); 
roiManager("Add");//#1
...
selectWindow("Summary of Pax");//Table.rename("Summary", SaveName+"_Cen-Pax");
Table.save(outputData+SaveName+"_Cen-Sum-Pax.csv");Table.deleteRows(0, 15, "Summary of Pax");
selectWindow("Mask of Pax");run("Invert LUT");
...
selectWindow("Summary of Pax");//Table.rename("Summary", SaveName+"_Peri-Pax");

Table.save(outputData+SaveName+"_Peri-Sum-Pax.csv");Table.deleteRows(0, 15, "Summary of Pax");
selectWindow("Mask of Pax");run("Invert LUT");
saveAs("Tiff", outputImages+"Pax-Peri-mask_"+SaveName);close();


roiManager("Reset");
run("Close All");run("Clear Results");Table.deleteRows(0, 15, "Summary of Pax");
}